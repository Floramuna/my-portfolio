...
const Navbar = () => {
    ...
    return (<header className={`fixed w-full top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/95 shadow-md backdrop-blur-sm py-2' : 'py-4 bg-transparent'}`}>
      <div className="container mx-auto px-4 flex justify-between items-center">
        <react_router_dom_1.Link to="/" className="text-2xl font-bold gradient-text" onClick={closeMenu}>
          Port<span className="text-purple">folio</span>
        </react_router_dom_1.Link>

        {/* Desktop Navigation */}
        <nav className="hidden md:flex space-x-8">
          {navLinks.map((link) => (<react_router_dom_1.Link key={link.title} to={link.path} className={`font-medium transition-all duration-300 hover:text-purple ${location.pathname === link.path ? 'text-purple' : 'text-gray-700'}`}>
              {link.title}
            </react_router_dom_1.Link>))}
        </nav>
      ...
      <div className={`fixed top-16 left-0 right-0 bg-white shadow-lg transition-transform duration-300 transform md:hidden ${isOpen ? 'translate-y-0' : '-translate-y-full'}`}>
        <div className="container mx-auto px-4 py-4 flex flex-col space-y-4">
          {navLinks.map((link) => (<react_router_dom_1.Link key={link.title} to={link.path} className={`font-medium p-2 transition-colors duration-300 hover:bg-gray-100 rounded ${location.pathname === link.path ? 'text-purple' : 'text-gray-700'}`} onClick={closeMenu}>
              {link.title}
            </react_router_dom_1.Link>))}
        </div>
      </div>
