          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-black/40 flex items-end pb-12 md:pb-20 px-4 md:px-8">
            <div className="text-white max-w-4xl w-full mx-auto">
              ...
              <div className="flex flex-wrap gap-2">
                {project.tags.map(tag => (<span key={tag} className="px-3 py-1 bg-white/20 rounded-full text-sm backdrop-blur-sm">
                    {tag}
                  </span>))}
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-2">
              ...
              <div className="prose max-w-none">
                {project.longDescription.split('\n').map((paragraph, i) => (<p key={i} className="mb-4 text-gray-700">{paragraph}</p>))}
              </div>

              {/* Gallery */}
              {project.gallery.length > 0 && (<div className="mt-16">
                  <h3 className="text-2xl font-bold mb-8">Project Screenshots</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {project.gallery.map((image, index) => (<div key={index} className="rounded-lg overflow-hidden shadow-md hover:shadow-xl transition-shadow">
                        <img src={image} alt={`${project.title} screenshot ${index + 1}`} className="w-full h-auto object-cover" loading="lazy"/>
                      </div>))}
                  </div>
                </div>)}
            </div>
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      {project.tags.map(tag => (<span key={tag} className="px-3 py-1 bg-purple/10 text-purple rounded-full text-sm">
                          {tag}
                        </span>))}
                    </div>
                  </div>
